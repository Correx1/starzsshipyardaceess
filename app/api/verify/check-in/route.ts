...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!guard_code?.trim()) {
      return NextResponse.json({ error: "Security Guard authorization PIN is required to check in." }, { status: 400 });
    }

    // 1. Verify Security Guard code against security_guards table
    ...
    if (guardError || !guard) {
      return NextResponse.json({ error: "Invalid Security Guard authorization PIN." }, { status: 403 });
    }
